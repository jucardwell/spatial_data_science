...
-   Reproject the `nc_counties` and `aq` objects into CRS = 2264 (North Carolina State Plane, ft) (i.e. `nc_counties <- nc_counties |> st_transform(crs = 2264)`)
-   Execute a spatial join between `temp` and `nc_counties`. Drop geometry and aggregate the `aq` object to the county level (take the sum of the `total_obs` and `total_over_70` columns per county).
