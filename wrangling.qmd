...
ch_temp_agg <- ch_temp |>
  ...
  summarise(mean = mean(`Maximum Air Temperature (F)`),
            min = mean(`Maximum Air Temperature (F)`),
            max = mean(`Maximum Air Temperature (F)`))
```
...
sat_wide <- sat_data |>
  pivot_wider(
    names_from = year,
    values_from = c(avg_sat_score, pct_sat_participation),
    names_sep = "_"
  )

sat_wide1 <- sat_wide |> mutate(diff = avg_sat_score_2021 - avg_sat_score_2019)
