...
acs_tract_nc <- st_read("https://drive.google.com/uc?export=download&id=1cnz4xgdDRZvlXzN3IyvCxOETRWfrpw-0") |> filter(COUNTYFP %in% c("135", "183", "063"))
```
...
```{r}
#lag model
fit.lag<-lagsarlm(pct_under_poverty_level ~ pct_no_health_insur,  
                  data = acs_tract_nc, 
                  listw = nbw) 
#see results
summary(fit.lag)
...
#error model
fit.error<-errorsarlm(pct_under_poverty_level ~ pct_no_health_insur,  
                  data = acs_tract_nc, 
                  listw = nbw)  
#see results
summary(fit.error)
